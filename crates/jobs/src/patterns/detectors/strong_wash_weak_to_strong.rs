...
impl PatternDetector for StrongWashWeakToStrongDetector {
    ...
    fn detect(
        ...
        for big_idx in (end.saturating_sub(6)..end.saturating_sub(2)).rev() {
            ...
            if big_change < 0.08 || !is_bullish(series, big_idx)? || big_volume < big_vol_ma * 1.5 {
                continue;
            }
            let wash_idx = big_idx + 1;
            let wash_open = series.open_at(wash_idx)?;
            let wash_volume = series.volume_at(wash_idx)?;
            let wash_time = series.time_at(wash_idx)?;
            if wash_idx >= end || !is_bearish(series, wash_idx)? || wash_volume < big_volume * 1.2 {
                continue;
            }
            let reversal_end = (wash_idx + 3).min(end - 1);
            for reversal_idx in wash_idx + 1..=reversal_end {
                let reversal_close = series.close_at(reversal_idx)?;
                let reversal_time = series.time_at(reversal_idx)?;
                if !is_bullish(series, reversal_idx)? {
                    continue;
                }
                if reversal_close <= big_close && reversal_close <= wash_open {
                    continue;
                }
                ...
                if reversal_idx < latest_idx
                    && (reversal_idx + 1..end)
                        .any(|idx| series.close_at(idx).is_some_and(|close| close <= big_close))
                {
                    continue;
                }
                if (reversal_idx..end)
                ...
                return Some(signal(
                    ...
                    json!({
                        "big_candle_date": big_time.format("%Y-%m-%d").to_string(),
                        "wash_date": wash_time.format("%Y-%m-%d").to_string(),
                        "reversal_date": reversal_time.format("%Y-%m-%d").to_string(),
                        "big_candle_volume_ratio": big_volume / big_vol_ma.max(1e-6),
                        "wash_volume_ratio": wash_volume / big_volume.max(1e-6),
