...
impl PatternDetector for TrendResonanceReversalDetector {
    ...
    fn detect(&self, series: &BarSeries, indicators: &SeriesIndicators) -> Option<PatternSignal> {
        ...
        Some(signal(
            self.id(),
            series,
            series.bars[rsi_day].time,
            0.8,
            &["resonance", "reversal", "rsi"],
            "RSI突破、均线金叉与MACD金叉在短窗口内共振，符合趋势共振反转结构。",
            json!({
                "key_date": series.bars[rsi_day].time.format("%Y-%m-%d").to_string(),
                "key_date_type": "RSI突破日",
                "rsi_breakout_day": series.bars[rsi_day].time.format("%Y-%m-%d").to_string(),
                "ma_cross_day": series.bars[ma_day].time.format("%Y-%m-%d").to_string(),
                "macd_cross_day": series.bars[macd_day].time.format("%Y-%m-%d").to_string(),
                ...
                "close": series.bars[rsi_day].close,
