...
impl PatternDetector for TrendStartDetector {
    ...
    fn detect(&self, series: &BarSeries, indicators: &SeriesIndicators) -> Option<PatternSignal> {
        ...
        let boll_cross = yesterday_close < boll_yesterday && today_close > boll_today;
        let volume_ok = volume_ratio > self.volume_ratio;
        if macd_cross && boll_cross && is_bullish(series, idx)? && today_close > ma5 && volume_ok {
            return Some(signal(
                self.id(),
                series,
                today_time,
                ...
                json!({
                    "key_date": today_time.format("%Y-%m-%d").to_string(),
